...
### В данной версии решены все основные задачи и все дополнительные кроме нагрузочного тестирования, на ветке base-tasks только основные
...
также в папке postman есть коллекция тестовых запросов(для основных и с доп роутами более новая), делал для себя, но т.к. я не знаю как проводиться проверка, может пригодится:)

## Дополнительные задания

###  Выполненные задания

1. **Эндпоинт статистики** — `/v1/api/stats/assignments` 
   - Агрегированная статистика по количеству назначений на пользователей и ревьюверов в PR.

2. **Массовая деактивация** — `/v1/api/team/bulkDeactivate`
   - Массовое отключение пользователей команды. 
   - Для каждого открытого PR ревьюверы автоматически перекидываются на активных коллег.

3. **Конфигурация линтера** — `.golangci.yml`
   - Включены проверки: govet, revive, gofmt, goimports, ineffassign, errcheck, staticcheck.

4. **Интеграционное тестирование** — `test/integration/integration_test.go` + `internal/http/handlers/stats_handler_test.go`

### Запуск тестов

```bash
# Запуск интеграционных тестов (требует работающей PostgreSQL БД)
go test -v ./test/integration

# Список всех интеграционных тестов
go test -v ./test/integration -list='.*'
...
- используется `golangci-lint` (конфиг `.golangci.yml`)
- включены базовые проверки: `govet`, `revive`, `gofmt`, `goimports`, `ineffassign`, `errcheck`, `staticcheck`
- запуск: `golangci-lint run ./...`

`revive` контролирует стиль, `govet` — потенциальные ошибки, `errcheck` обеспечивает обработку ошибок, `staticcheck` находит узкие места производительности/баги. Форматтеры (`gofmt`, `goimports`) гарантируют единый стиль.
